...
weeklyCommonGoals (일정관리 모든유저 볼수있는 할일)

daily_logs (매일할일 지금은 삭제됨)


  - 알림 및 시스템 
news (뉴스)
notifications (공지사항 및 알림등록 )
chats (채팅)
users ( 유저관리 )


  - 오픈마켓 
market_products (오픈마켓 상품정보)
markets (오픈마켓 총 아이디관리)
open_market (마켓목록)
daily_summaries (오픈마켓 택배출고기록)


  - 창고관리 카테고리
inventories ( 생성된 인벤토리)
logistics (물류기기 )
partners (거래처)
teams (작업팀관리)
transports (운송사 관리)
warehouses ( 창고관리 )
products (상품과 , 상품별 등록된 물류기기)

  - 창고관련 실직적인데이터 정리
logistics_movements (물류기기 이동관련)
shipping (정산서)
warehouse_inventory (창고별 입고 생산 선별대기 선별완료 등 창고아이템정보)

생성해야될 컬렉션 
- 푸드엔관련
- 생성된 인벤토리를 그냥 등록하면 될듯 


 ### 10-10일 해야될것
  - 오픈마켓 반품내역 추가 (생성, 수정, 삭제, 조회)
  - 반품된 금액과 수량을 택배에서 빼주는 로직 추가 
  - 반품된 수량은 총 택배수량에 + 되어야 한다. (금액은 빼주고)
  - 오픈마켓 카테고리 생성 , 카테고리 컬렉션에 내부 컬렉션 market에 만들기
  - 카테고리 등록 페이지 만들기 , 대량등록 
  - 상품생성, 관리에 카테고리 추가 
  - 상품 조회에 카테고리 추가 

  완료 
  - 상품조회 페이지 뒤로가기시 기존페이지와 정보 남아있지 않는것 남아있게 수정완료 
  - 결산 레포트에는 달력이 이전 달력이 설정되도록 기본 디폴트 값으로 변경 

  #### 새로운것 추가해야될것 
    - 아르고만 따로 수정 저장해야되는데 이거 어떻게 해야될까 ?
...
yarn build
git add .
git commit -m 'v2.6.255'
git branch v2.6.255
git push -u origin v2.6.254
###### git
...
1. 1톤차량 입고부터 수정, 
 ...
 2. 거래처선택완료, 
 3. 카테고리 컬렉션을 만들어서 카테고리를 관리하자 , 
 4. 운송정보입력 ,작업팀을 추가하여, 운송비내역을 추가함
 5. 바렛트 선택, (버튼으로 변경)
 6. 상품선택(이미선택되어있는 입고빼버리기)
 7. 상품을 + 버튼으로 추가하여 배열로 등록될수 있도록 하기 , 직관적


2. 5톤차량 입고 
 1. 위에랑 같은데 , 각 상품별로 , 이미지 등록할수 있도록 하기 , 
...
# 2.6.255
  - chat_windows bugifx
  - PartnerInfoComponent Ui 변경
  - firebase Cloud Functions 적용
  - 전체 코드 Serverless Architecture 작업 중
