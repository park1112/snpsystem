...
export const updateWarehouseInventory = async (warehouseId, logisticsItemId, logisticsItemName, quantity, isInbound, movementId) => {
...
export const updateMovement = async (warehouseId, logisticsItemId, oldMovement, newMovement, newWarehouseId = null) => {
    if (newWarehouseId && newWarehouseId !== warehouseId) {
        // 다른 창고로 이동하는 경우
        console.log('이동하는정보:', warehouseId, logisticsItemId, oldMovement,);
        await updateOrDeleteMovement(warehouseId, logisticsItemId, oldMovement, null);
        await updateWarehouseInventory(newWarehouseId, logisticsItemId, newMovement.name, newMovement.quantity, newMovement.type === 'inbound', newMovement.movementId);
    } else {
        // 같은 창고 내에서 수정하는 경우
        const warehouseRef = doc(db, 'warehouses', warehouseId);
