...
const generateSummaryPDF = async (data) => {
    const { summary, totalQuantity, totalPrice, date, markets } = data;
    ...
    doc.setFontSize(20);
    doc.text(`상세보기 - ${markets || 'Unknown Market'}`, 14, 20);

    doc.setFontSize(12);
    doc.text(`날짜: ${date ? dayjs(date.toDate()).format('YYYY-MM-DD HH:mm') : 'N/A'}`, 14, 30);
    ...
    doc.save(`${markets || 'Unknown_Market'}_${date ? dayjs(date.toDate()).format('YYYY-MM-DD') : 'N/A'}`);
};
