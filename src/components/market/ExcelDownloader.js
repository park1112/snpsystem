...
const ExcelDownloader = ({ itemList, productMappings, selectedMarket, markets }) => {
    const handleDownload = () => {
        ...
        const filteredData = itemList[selectedMarket]
            .filter(item => marketMapping[item.옵션ID || item.옵션정보 || item.상품번호 || item.옵션 || item.옵션번호 || item.상품코드])
            .map(item => {
                const mapping = marketMapping[item.옵션ID || item.옵션정보 || item.상품번호 || item.옵션 || item.옵션번호 || item.상품코드];
                ...
                return {
                    '예약구분': '',
                    '집하예정일': '',
                    '받는분성명': item.수취인이름 || item.수취인명 || item.주문자명 || item.수령인명 || item.받는사람,
                    '받는분전화번호': item.구매자전화번호 || item.수취인연락처1 || item.전화번호 || item['수령인 휴대폰'] || item.배송지연락처,
                    '받는분기타연락처': '',
                    '받는분우편번호': '',
                    '받는분주소': item['수취인 주소'] || item.통합배송지 || item.주소 || item.배송지,
                    '운송장번호': '',
                    '고객주문번호': '',
                    ...
                    // '기본운임': mapping.price,
                    '기본운임': 0,
                    '배송메세지1': item.배송메세지 || item.배송요청메모 || item.요청사항 || item['배송시 요구사항'] || item['배송위치 / (출입정보) / (배송요청사항)'],
                    '배송메세지2': '',
                };
            });
