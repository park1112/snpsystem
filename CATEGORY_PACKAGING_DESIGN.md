...
| 요구사항 | 상세 |
|---------|------|
| 마켓별 매입가 구성 | 상품가격, 박스비, 배송비, 부자재비 조합 설정 |
| 비용 항목 선택 | 마켓별로 포함할 비용 항목 체크박스 방식 |
| 유연한 공식 변경 | 추후 비용 구성 공식 변경 가능 |
| 마켓별 수수료 적용 | 각 마켓의 수수료율 별도 관리 |
...
#### 4.1.3-1 오픈마켓 확장 (`open_market`) - 비용 구성 설정 추가
```javascript
{
  id: "market_001",
  name: "쿠팡",
  ...
  // === 비용 구성 설정 (신규) ===
  costFormula: {
    // 매입가에 포함할 비용 항목 (체크박스 방식)
    includeProductPrice: true,    // 상품가격 (필수, 항상 true)
    includeBoxCost: true,         // 박스비
    includeShippingCost: true,    // 배송비
    includePackagingCost: true,   // 부자재비 (보냉팩 등)

    // 별도 정산 항목 (매입가에 미포함)
    separateItems: {
      advertisingCost: true,      // 광고비 별도
      marketFee: true             // 수수료 별도 (항상 별도)
    },

    // 공식 설명 (자동 생성)
    formulaDescription: "상품가격 + 박스비 + 배송비 + 부자재비",
  ...
  // === 추가 설정 ===
  defaultShippingCost: 3000,      // 기본 배송비 (마켓별 설정 가능)
  advertisingRate: 0,             // 광고비율 (%) - 선택적
...
// 시나리오 2: 네이버 (배송비 별도)
{
  ...
  costFormula: {
    includeProductPrice: true,
    includeBoxCost: true,
    includeShippingCost: false,   // 배송비 별도 정산
    includePackagingCost: true
  }
  // 매입가 = 상품가격 + 박스비 + 부자재비
}
...
// 시나리오 3: 도매 (상품가격만)
{
  ...
  costFormula: {
    includeProductPrice: true,
    includeBoxCost: false,        // 박스비 별도
    includeShippingCost: false,   // 배송비 별도
    includePackagingCost: false   // 부자재비 별도
  }
  // 매입가 = 상품가격만
}
```
...
```javascript
{
  ...
  marketPricing: {
    "market_001": {                // 쿠팡
      ...
      // 마켓별 매입가 (공식에 따라 계산)
      marketPurchasePrice: 13900,  // 상품가격 + 박스비 + 배송비 + 부자재비

      // 마켓별 최종 비용
      marketFeeRate: 9,
      marketFee: 1620,             // 판매가 × 수수료율
      advertisingCost: 500,
      finalCost: 16020,            // 매입가 + 수수료 + 광고비

      // 마켓별 손익
      sellingPrice: 18000,
      margin: 1980,
      profitRate: 11.0,
      profitStatus: "PROFIT"       // PROFIT | BREAK_EVEN | WARNING | LOSS
    },

    "market_002": {                // 네이버 (배송비 별도)
      marketId: "market_002",
      marketName: "네이버 스마트스토어",
      ...
      marketPurchasePrice: 10900,  // 상품가격 + 박스비 + 부자재비 (배송비 제외)
...
[2단계: 마켓별 매입가 계산] ★ 핵심
┌─────────────────────────────────────────────────────────────────────────────┐
│ 매입가 = 마켓 비용 공식에 따라 선택적 합산                                   │
│                                                                             │
│ 예시:                                                                       │
│ ┌─────────────────┬─────────────────────────────────────┬─────────────────┐│
│ │ 마켓            │ 포함 항목                           │ 매입가 계산      ││
│ ├─────────────────┼─────────────────────────────────────┼─────────────────┤│
│ │ 쿠팡            │ 상품 + 박스 + 배송 + 부자재          │ 13,900원        ││
│ │ 네이버          │ 상품 + 박스 + 부자재 (배송 별도)     │ 10,900원        ││
│ │ 도매처          │ 상품만 (나머지 별도)                 │ 10,000원        ││
│ └─────────────────┴─────────────────────────────────────┴─────────────────┘│
└─────────────────────────────────────────────────────────────────────────────┘
...
```javascript
// 마켓별 비용 공식에 따른 매입가 계산
function calculateMarketPurchasePrice(product, market) {
  const formula = market.costFormula;
  let purchasePrice = 0;

  // 상품가격 (항상 포함)
  if (formula.includeProductPrice) {
    purchasePrice += product.costBreakdown.purchasePrice;  // kg당 가격 × 중량
  }

  // 박스비 (선택적)
  if (formula.includeBoxCost) {
    purchasePrice += product.costBreakdown.boxCost;
  }

  // 배송비 (선택적)
  if (formula.includeShippingCost) {
    purchasePrice += product.costBreakdown.shippingCost;
  }

  // 부자재비 (선택적)
  if (formula.includePackagingCost) {
    purchasePrice += product.costBreakdown.packagingCost;
  }
...
// 사용 예시
const coupangPrice = calculateMarketPurchasePrice(product, coupangMarket);
// 결과: 10,000 + 500 + 3,000 + 400 = 13,900원

const naverPrice = calculateMarketPurchasePrice(product, naverMarket);
// 결과: 10,000 + 500 + 0 + 400 = 10,900원 (배송비 별도)
```

### 4.3.2 비용 공식 변경 시 영향 분석
...
│  │                                                                    │  │
│  │  예시 (양파(소) 5kg 기준):                                        │  │
│  │  매입가 = 10,000 + 500 + 3,000 + 400 = 13,900원                   │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                                                                         │
│  ⚠️ 공식 변경 시 해당 마켓의 모든 상품 손익이 재계산됩니다.             │
│                                                                         │
│                                        [취소] [미리보기] [저장]          │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
...
#### 4.5.7 마켓별 손익 비교 페이지 (신규)

```
┌─────────────────────────────────────────────────────────────────────────┐
│  📊 마켓별 손익 비교                              상품: [양파(소) 5kg ▼] │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  📦 상품 기본 정보                                                      │
│  ├── 카테고리: 양파(소)                                                │
...
│  📋 마켓별 비용 비교                                                    │
│  ┌──────────┬─────────┬─────────┬─────────┬────────┬────────┬───────┐ │
│  │ 마켓     │ 매입가   │ 수수료  │ 광고비  │ 총비용  │ 판매가 │ 손익  │ │
│  ├──────────┼─────────┼─────────┼─────────┼────────┼────────┼───────┤ │
│  │ 쿠팡     │ 13,900  │ 1,620   │ 500    │ 16,020 │ 18,000 │🟢+11% │ │
│  │ (전체포함)│         │ (9%)    │        │        │        │       │ │
│  ├──────────┼─────────┼─────────┼─────────┼────────┼────────┼───────┤ │
│  │ 네이버   │ 10,900  │ 880     │ 300    │ 12,080 │ 16,000 │🟢+25% │ │
│  │ (배송별도)│         │ (5.5%)  │        │        │        │       │ │
│  ├──────────┼─────────┼─────────┼─────────┼────────┼────────┼───────┤ │
│  │ 11번가   │ 13,400  │ 1,470   │ 200    │ 15,070 │ 15,000 │🔴-0.5%│ │
│  │ (부자재별도)│       │ (9.8%)  │        │        │        │       │ │
│  └──────────┴─────────┴─────────┴─────────┴────────┴────────┴───────┘ │
│                                                                         │
│  💡 분석 결과                                                           │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │ • 가장 수익률 높은 마켓: 네이버 (+24.5%)                          │  │
│  │ • 손실 발생 마켓: 11번가 (-0.5%) → 판매가 15,300원 이상 권장      │  │
│  │ • 배송비 별도인 마켓이 수익률에 유리함                            │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
- [ ] Firestore 컬렉션 생성/확장
  ...
  - `open_market` 확장 (+ costFormula 필드)
- [ ] 기본 CRUD 서비스 함수 작성
- [ ] 마켓별 매입가 계산 유틸리티 함수 작성
- [ ] 가격 계산 유틸리티 함수 작성
...
- [ ] 마켓 비용 설정 페이지 (`MarketCostSettingPage`) - 신규
  - 비용 공식 체크박스 UI
  - 공식 변경 미리보기
  - 변경 시 영향 분석
...
- [ ] 상품 손익 분석 페이지 (`ProductProfitAnalysisPage`)
  - 손실 상품 경고
  - 권장 조치 표시
- [ ] 마켓별 손익 비교 페이지 (`MarketProfitComparisonPage`) - 신규
  - 같은 상품의 마켓별 비용/손익 비교
  - 최적 마켓 추천
...
| 영역 | 변경 내용 |
|------|----------|
| 데이터 | 6개 신규 컬렉션 + `market_products` 대폭 확장 + `open_market` 확장 |
| UI | 8개 신규 관리 페이지 + 대시보드 + 가격 승인 + 마켓 비용 설정 |
| 로직 | 출고 시 자동 집계 + 재고 차감 + 마켓별 비용 계산 + 가격 승인 워크플로우 |

### 7.2 신규/확장 컬렉션 요약

| 컬렉션 | 용도 | 상태 |
|--------|------|------|
| `product_categories` | 카테고리 관리 + kg당 기준가 | 신규 |
| `packaging_materials` | 포장자재 종류 + 단가 | 신규 |
| `packaging_inventory` | 포장자재 재고 | 신규 |
| `packaging_movements` | 포장자재 입출고 기록 | 신규 |
| `price_change_requests` | 가격 변동 승인 요청 | 신규 |
| `price_change_history` | 가격 변동 이력 | 신규 |
| `open_market` | 마켓별 비용 구성 공식 (costFormula) | 확장 |
| `market_products` | 마켓별 매입가 (marketPricing) | 확장 |
...
│                                                                 │
│  예시:                                                          │
│  • 쿠팡: 전체 포함 → 매입가 13,900원                            │
│  • 네이버: 배송비 별도 → 매입가 10,900원                        │
│  • 도매: 상품가격만 → 매입가 10,000원                           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
